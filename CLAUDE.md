...
- **One state object per site, one load + one save per run** (`state/wp-seen-<site>.json`: `ids`, `cursor`, `hashes`, `fires`, `perPost`). New-posts run **first** so a new article reaches Airtable before any update for it, and so firing it seeds the baseline hash that suppresses its own publish-echo.
